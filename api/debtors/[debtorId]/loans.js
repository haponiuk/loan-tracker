...
export default async function handler(request, response) {
    ...
    try {
        const input = normalizeLoanInput(request.query.debtorId, parseRequestBody(request.body));
...
function normalizeLoanInput(debtorIdValue, input) {
    ...
    return {
        ...
        date: normalizeDate(input.date, 'Вкажи дату позики.'),
        dueDate: normalizeOptionalDate(input.dueDate),
        notes: notes || null,
        files: normalizeFileInput(input.files),
    };
}
...
async function insertLoan(input) {
    ...
    const {error} = await supabase.from('loans').insert(insertPayload);
    ...
    const retry = await supabase
        .from('loans')
        .insert({
            id: await getNextRecordId('loans'),
            ...insertPayload,
        });
