...
export default async function handler(request, response) {
    ...
    if (!supabase) {
        response.status(500).json({error: 'Не задано серверні Supabase env-змінні для завантаження фото.'});
        return;
    }
    ...
    try {
        ...
        if (!contentType.startsWith('image/')) {
            response.status(400).json({error: 'Можна завантажувати тільки зображення.'});
            return;
        }
